...
extern EthernetVars enet0;
EthernetVars *enetVar = &enet0;

BOOL SetTimeNTPFromPool(bool debug);
...
BOOL SetTimeNTP()
{
    if (enetVar && !(enetVar->LinkActive())) { OSTimeDly((uint16_t)(1.5 * TICKS_PER_SECOND)); }

    printf("Acquiring time from NTP server... ");
    ...
    {
        printf("Failed\r\n");
        printf("Verify you have correct IP address, mask, gateway AND DNS set\r\n");
    }
    return FALSE;
}
...
/*-------------------------------------------------------------------
   ...
   Coordinated Universal Time. It is of type uint32_t, and used for time
   calculations.
...
/* Synchronize the RTC with the system time. */
    ...
    int n = 1;
#endif
    if (n == 0) { printf("Set system time from RTC\r\n"); }
    else
    {
        printf("Error: could not set system time from RTC\r\n");
    }
}
...
/*-------------------------------------------------------------------
  SetNamedTimeZone() - Look up a zone in the system TZRecords[] table
  by its formal Name (e.g. "Pacific Standard Time") and apply it with
  tzsetchar(). The table (75+ world zones) lives in
  nbrtos/source/timezones.cpp and is exposed through <timezones.h>.

  Returns TRUE if the name was found and the zone set, FALSE otherwise
  (in which case the current zone is left unchanged).
 -------------------------------------------------------------------*/
BOOL SetNamedTimeZone(const char *zoneName)
{
    for (const TimeZoneRecord *tz = TZRecords; tz->Posix != nullptr; tz++)
    {
        if (strcmp(tz->Name, zoneName) == 0)
        {
            tzsetchar(tz->Posix);
    ...
    printf("Time zone \"%s\" not found; zone left unchanged\r\n", zoneName);
    return FALSE;
}

/*-------------------------------------------------------------------
  ListTimeZones() - Print every zone in the TZRecords[] table to the
  serial console (index, formal Name, and description). Useful for
  discovering the exact Name string to pass to SetNamedTimeZone().
 -------------------------------------------------------------------*/
void ListTimeZones()
{
    int i = 0;
    for (const TimeZoneRecord *tz = TZRecords; tz->Posix != nullptr; tz++, i++)
    {
        printf("%3d: %-32s %s\r\n", i, tz->Name, tz->Description);
    }
}
